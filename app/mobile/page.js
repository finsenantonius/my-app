...
export default function Home() {
  ...
  return (
    <main>
      ...
      <div className="flex flex-col pt-6 justify-center items-center mt-16">
        <div className="w-[500px]">
          ...
          <div className="mb-7">
            <form className="flex items-center py-3 px-4 rounded-[12px] w-[200px] bg-simple-gray">
              <input type="text" placeholder="Enter username" className="w-[200px] bg-transparent outline-none" value={username} onChange={(e) => setUsername(e.target.value)}>
              </input>
              {/* <span className="w-[1px] h-[28px] bg-simple-gray"></span> */}
            </form>
          </div>
          <div>
            <div className="grid gap-6 grid-cols-1">
              {data.map((item) => {
                return (
                  item.ammount === "Custom" && selected?.ammount === "Custom" ? (
                    <div 
                    key={item.ammount} 
                    onClick={() => handleSelect(item)} 
                    className={`h-[96px] py-6 rounded flex flex-col items-center justify-center ${selected?.ammount === item.ammount ? 'border-2 border-simple-yellow' : 'border border-simple-gray-3'}`}
                  >
          ...
          {selected?.ammount === "Custom" ? (
            <div className="mt-2">
              <p className="tracking-[1px]">Total <span className="font-bold tracking-normal">IDR {total}</span></p>
            </div>
          ) : (
            <div className="mt-2">
